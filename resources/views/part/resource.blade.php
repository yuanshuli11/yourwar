...
<div class="home-content">
    <div class="part-body">
     {{--    @foreach($building as $key=>$item)
        <div  v-show="buildingPage=='{{ceil(($key+1)/8)}}'"  class="row show-building-item" style="display: none;">
            <div class="col-xs-5">{{$item['name']}} @if($item['level'])（{{$item['level']}}级） @endif</div>
            <div class="col-xs-2 item-a"> @if($item['name_type']!=30) <a>进入</a> @else   <a v-on:click="build({{$item['id']}})">建造</a>  @endif</div>
            <div class="col-xs-3 item-a"> @if($item['level']&&$item['level']<=10)<a  v-on:click="upLevel({{$item['id']}})">升级</a> @endif</div>
            <div class="col-xs-2 item-a"> @if($item['name_type']!=30) <a>拆除</a> @endif</div>
        </div>
        @endforeach --}}
            ...
            <div v-for="(build,index) in building"  v-show="buildingPage==Math.ceil((index+1)/8)"  class="row show-building-item" style="display: none;">
                ...
                <div class="col-xs-2 item-a">  <a v-if="build.name_type!=40">进入</a>   <a v-if="build.name_type==40"   v-on:click="buildUp(build.id)">建造</a></div>
                ...
                <div class="col-xs-2 item-a"> <a v-if="build.name_type!=40">拆除</a></div>
            </div>
        ...
        <div class="modal-dialog modal-sm" role="document">
            <div  v:if="nowBuildingInfo" class="modal-content">
                ...
                <div class="modal-footer">
                    <span v-if="nowBuildPage>0&&buildType=='build'" v-on:click="buildPageChange('sub')"  class="btn btn-sm btn-info" v >上页</span>
...
<script type="text/javascript">
    ...
    var app = new Vue({
        ...
        data: {       
            ...
            buildingMax:{{ceil(count($building)/8)}},
            canBuildigInfo:{!!json_encode($canBuild)!!},
            userNowStatus:{!!json_encode($userNowStatus)!!},
            nowBuildPage:0,
            maxBuildPage:{{count($canBuild)-1}},
        ...
        methods: {
            ...
            upLevel:function(area_id){
                ...
                $.ajax({
                    url:"{{ asset('/api/build/v1/getBuildUpInfo') }}",
                    method:"POST",
                    data:data,
            ...
            realBuild:function(){
                ...
                if(this.canBuild==0&&this.areaId>0){
                    ...
                    if(CAN_SUBMIT){
                        CAN_SUBMIT = false;
                        $.ajax({
                            url:"{{ asset('/api/build/v1/building') }}",
                            method:"POST",
                            data:data,
                            success:function(result){
                                CAN_SUBMIT = true;                            

...
<div class="show-add">
    ...
    <form class="form-horizontal" method="POST" action="{{ asset('/sysadmin/object/addDetail/'.$object['id'])  }}">
        {{ csrf_field() }}
        <div class="form-group">
            <label  class="col-xs-1 control-label">名称</label>
            <div class="col-xs-2">
              <input  class="form-control" value="{{$object['name']}}" readonly="readonly" placeholder="名称">
            </div>
        ...
        <div class="form-group">
            ...
            <div class="col-xs-2">
                <select class="form-control"  name="need_building1">
                    ...
                    <option value="{{$item['name_type']}}">{{$item['name']}}</option>
                    @endforeach
                </select>
            </div>
            <div class="col-xs-2">
        ...
        <div class="form-group">
            ...
            <div class="col-xs-2">
                <select class="form-control"  name="resource_ability">
                    <option value="0">无</option>
                    @foreach(config('ability.resource') as $key=>$item)
...
<script type="text/javascript">
        var app = new Vue({
            el: '.show-add',
            data: {   
                objectDetail:{!! json_encode($objectDetail) !!},
                resourceName:{!!json_encode(config('setting.resourceName'))!!},
                object:{!! json_encode($object) !!},
                ability:{!! json_encode(config('ability.resource')) !!},
